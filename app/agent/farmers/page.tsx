...
export default function AgentFarmers() {
  ...
  useEffect(() => { fetch("/api/catalog").then(response => response.ok ? response.json() : null).then(data => setFarmers((data?.data?.users ?? []).filter((user: User) => user.role === "farmer"))).catch(() => setFarmers([])); }, []);
  return (
        ...
        {farmers.map(f => (
          <div key={f.id} style={{ background: "white", border: "1px solid var(--line)", borderRadius: "10px", padding: "22px" }}>
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              <button className="btn btn-primary btn-sm" style={{ flex: 1 }} onClick={() => router.push(`/messages?chat=${f.id}`)}>Message</button>

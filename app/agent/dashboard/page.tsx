...
import { MOCK_AGGREGATIONS, MOCK_PRODUCE, MOCK_ORDERS, MOCK_FARMERS } from "@/lib/data";
import { formatPrice, formatDate, getCommodityName } from "@/lib/data";

export default function AgentDashboard() {
  const { user } = useAuth();
  const myAggregations = MOCK_AGGREGATIONS.filter(a => a.agentId === user?.id);
          ...
          <table className="produce-table">
            ...
            <tbody>
              {myAggregations.map((a, i) => {
                const farmer = MOCK_FARMERS.find(f => f.id === a.farmerId);
                const produce = MOCK_PRODUCE.find(p => p.id === a.produceId);
                return (
                  <tr key={i}>
                    <td className="commodity-name">{farmer?.name || "Unknown"}</td>
                    <td>{produce ? getCommodityName(produce.commodityId) : "N/A"} <span style={{ color: "#708077", fontWeight: 400 }}>{produce?.variety}</span></td>
                    <td>{a.quantity.toLocaleString()} {produce?.packaging.split(" ")[1] || "units"}</td>
